...
import no.nav.helse.flex.kafka.RAPID_TOPIC
...
class LagreKafkaMeldingerTest : FellesTestOppsett() {
    ...
    @Autowired
    lateinit var vedtaksperiodeFunksjonellFeilRepository: VedtaksperiodeFunksjonellFeilRepository

    @Test
    @Order(1)
    fun `Tar imot søknad ider`() {
        sykepengesoknadIdRepository.count() `should be equal to` 0
        kafkaProducer.send(ProducerRecord(RAPID_TOPIC, UUID.randomUUID().toString(), soknad))
    ...
    fun `Tar imot vedtaksperiodeEndretEvent og lagrer i to tabeller`() {
        ...
        kafkaProducer.send(ProducerRecord(RAPID_TOPIC, UUID.randomUUID().toString(), vedtaktaksperiodeEndret))
    ...
    fun `Tar imot vedtaksperiodeForkastetEvent og lagrer i to tabeller`() {
        ...
        kafkaProducer.send(ProducerRecord(RAPID_TOPIC, UUID.randomUUID().toString(), vedtaksperiodeForkastet))
    ...
    @Test
    @Order(4)
    fun `Tar imot aktivitetslogg ny aktivitet`() {
        vedtaksperiodeFunksjonellFeilRepository.count() `should be equal to` 0

        kafkaProducer.send(ProducerRecord(RAPID_TOPIC, UUID.randomUUID().toString(), aktivitetsloggNyAktivitet))

        await().atMost(20, TimeUnit.SECONDS).until {
            vedtaksperiodeFunksjonellFeilRepository.count() == 1L
        }

        val vedtaksperiodeFunksjonellFeilDbRecord = vedtaksperiodeFunksjonellFeilRepository.findAll().toList().first()
        vedtaksperiodeFunksjonellFeilDbRecord.vedtaksperiodeId `should be equal to` "fed26326-9784-4875-8beb-68769c9e3db8"
        vedtaksperiodeFunksjonellFeilDbRecord.melding `should be equal to` "Manglervilkårsgrunnlagfor2022-05-01"
        vedtaksperiodeFunksjonellFeilDbRecord.tidspunkt.shouldNotBeNull()

        vedtaksperiodeFunksjonellFeilRepository.count() `should be equal to` 1
    }
    ...
    @Suppress("ktlint:standard:max-line-length")
    val aktivitetsloggNyAktivitet =
        """{"@event_name":"aktivitetslogg_ny_aktivitet","fødselsnummer":"12345678910","aktiviteter":[{"id":"0e3bd909-9709-45f1-8605-5d2d7b61b64e","nivå":"FUNKSJONELL_FEIL","melding":"Manglervilkårsgrunnlagfor2022-05-01","tidsstempel":"2022-08-22T16:17:40.564","kontekster":[{"konteksttype":"Person","kontekstmap":{"fødselsnummer":"12345678910","aktørId":"1234567891011"}},{"konteksttype":"Vedtaksperiode","kontekstmap":{"vedtaksperiodeId":"fed26326-9784-4875-8beb-68769c9e3db8"}}]}],"@id":"2891faac-5bf3-46f2-92af-6e8f509d137e","@opprettet":"2022-08-22T16:17:40.625910785"}"""
}
