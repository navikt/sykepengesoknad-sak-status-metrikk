...
class LagreKafkaMeldingerTest : FellesTestOppsett() {
    ...
    fun `Tar imot vedtaksperiodeForkastetEvent og lagrer i to tabeller`() {
        ...
        val sykepengesoknadIdDbRecords = sykepengesoknadIdRepository.findAll().toList()
        sykepengesoknadIdDbRecords.size `should be equal to` 2

        sykepengesoknadIdDbRecords.map { it.sykepengesoknadAtId } `should contain` "110d1fd9-50cf-4e21-b5e3-dbf10665a777"
